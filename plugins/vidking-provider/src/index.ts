...
export interface VidKingPluginConfig {
  id?: string
  name?: string
  apiBaseUrl?: string
  decryptUrl?: string
  timeoutMs?: number
  maxStreams?: number
  /** VidKing server names, e.g. yoru, breach, vyse, fade, omen, raze */
  servers?: string[]
}
...
interface ServerEntry {
  ...
  /** Keep only sources with this quality (e.g. hdmovie English/Hindi split) */
  qualityFilter?: string
}

/**
 * Server lineup from the VidKing web player (`/assets/VideoPlayer-*.js`):
 * Yoru=cdn, Cypher=downloader2, Breach=m4uhd, Neon=vsrc, Vyse/Fade=hdmovie,
 * Killjoy=meine (german), Omen=lamovie, Raze=superflix.
 */
const SERVER_TABLE: Record<string, ServerEntry> = {
  yoru: { endpoint: 'cdn/sources-with-title' },
  cypher: { endpoint: 'downloader2/sources-with-title' },
  breach: { endpoint: 'm4uhd/sources-with-title' },
  neon: { endpoint: 'vsrc/sources-with-title' },
  vyse: { endpoint: 'hdmovie/sources-with-title', qualityFilter: 'English' },
  killjoy: { endpoint: 'meine/sources-with-title', params: { language: 'german' } },
  fade: { endpoint: 'hdmovie/sources-with-title', qualityFilter: 'Hindi' },
  omen: { endpoint: 'lamovie/sources-with-title' },
  raze: { endpoint: 'superflix/sources-with-title' },
  // Raw endpoint aliases (also accepted via config as-is)
  cdn: { endpoint: 'cdn/sources-with-title' },
  m4uhd: { endpoint: 'm4uhd/sources-with-title' },
  hdmovie: { endpoint: 'hdmovie/sources-with-title' },
  lamovie: { endpoint: 'lamovie/sources-with-title' },
  superflix: { endpoint: 'superflix/sources-with-title' },
}

// ponytail: cypher/neon 404 + killjoy is german-only, so defaults are the live English servers
const DEFAULT_SERVERS = ['yoru', 'breach', 'vyse', 'fade', 'omen', 'raze']

/**
 * OMSS provider for VidKing (`vidking.net`) using its speedracelight
 * backends + enc-dec.app decryption (same scheme as the Videasy provider).
 */
export class VidKingProvider extends BaseProvider {
  ...
  constructor(config: VidKingPluginConfig = {}) {
    super()
    this.id = config.id ?? 'vidking'
    this.name = config.name ?? 'VidKing'
    this.BASE_URL = 'https://www.vidking.net'
    this.apiBaseUrl = (
      config.apiBaseUrl ??
      process.env.VIDKING_API_BASE_URL ??
      'https://api.speedracelight.com'
    ).replace(/\/$/, '')
    this.decryptUrl = (
      config.decryptUrl ??
      process.env.VIDKING_DECRYPT_URL ??
      'https://enc-dec.app/api/dec-videasy'
    ).replace(/\/$/, '')
    this.timeoutMs =
      config.timeoutMs ?? Number(process.env.VIDKING_TIMEOUT_MS ?? 18_000)
    this.maxStreams =
      config.maxStreams ?? Number(process.env.VIDKING_MAX_STREAMS ?? 4)
    this.servers = normalizeServers(
      config.servers ??
        process.env.VIDKING_SERVERS?.split(',') ??
        DEFAULT_SERVERS,
    )
  ...
  private async getSources(media: ProviderMediaObject): Promise<ProviderResult> {
    this.console.log('Fetching VidKing sources', media)

    try {
      const meta = await this.resolveMeta(media)
      if (!meta) {
        return this.emptyResult(
          'Could not resolve title/year/TMDB metadata for VidKing',
        )
      }

      const seed = await this.fetchSeed(meta.tmdbId)
      if (!seed) return this.emptyResult('Failed to obtain VidKing seed')
      ...
      const settled = await Promise.all(
        this.servers.map(async (server) => {
          try {
            ...
            return { server: server.endpoint, payload: null }
          }
        }),
      )
      ...
      if (!picked.length) {
        return this.emptyResult('No streams returned after VidKing decrypt')
      }
  ...
  private async fetchServer(
    ...
    for (const [k, v] of Object.entries(server.params ?? {})) qs.append(k, v)

    const encText = await this.fetchText(
      `${this.apiBaseUrl}/${server.endpoint}?${qs.toString()}`,
    )
    ...
    // ponytail: qualityFilter narrows (hdmovie English/Hindi split); fall back to all on mismatch
    if (server.qualityFilter) {
      const kept = sources.filter(
        (s) => String(s.quality || '').toLowerCase() === server.qualityFilter!.toLowerCase(),
      )
      if (kept.length) return { ...decrypted, sources: kept }
    }
    return decrypted
  }
...
function normalizeServers(values: string[]): ServerEntry[] {
  const out: ServerEntry[] = []
  for (const raw of values) {
    const v = String(raw || '').trim().toLowerCase()
    if (!v) continue
    // ponytail: raw "path/sources-with-title" endpoints pass through as-is
    if (v.includes('/')) {
      out.push({ endpoint: v.replace(/^\/+|\/+$/g, '') })
      continue
    }
    const entry = SERVER_TABLE[v]
    if (entry) out.push(entry)
  }
  return [...new Map(out.map((e) => [e.endpoint + (e.qualityFilter ?? ''), e])).values()]
}

function extractYear(media: ProviderMediaObject): string | null {
  const m = media as { year?: string | number; releaseYear?: string | number }
  const s = String(m.year ?? m.releaseYear ?? '').trim()
...
export function createOmssProviders(
  config: VidKingPluginConfig = {},
): BaseProvider[] {
  return [new VidKingProvider(config)]
}
